  ...
  const handleGenerateQuestions = async (category) => {
    ...
    try {
      const response = await fetch("/api/question", {
        ...
        body: JSON.stringify({ applyedJob, requiredCompetency, preferentialTreatment, idealTalent, selfIntroduction, category, maxQuestions: 3 }),
      });
    ...
    <div className="flex flex-col items-center justify-center min-h-screen pt-20 pb-8 bg-white w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-12 px-8 max-w-[1220px]">
        ...
        <button 
          ...
          onClick={() => handleGenerateQuestions("경험 및 직무 경험을 바탕으로 직무와의 적합성 및 전문성 평가")}
        >
        ...
        <button 
          ...
          onClick={() => handleGenerateQuestions("성격, 대인 관계 능력, 협업 태도 등을 평가하여 조직 적합성 판단")}
        >
          <div className="flex justify-center mb-4">
            <Image src={sprout} alt="sprout" width={42}/>
          </div>
          <h2 className="font-bold text-2xl text-gray-800 text-center">인성</h2>
          <p className="text-base text-gray-600 mt-2 text-center">
            성격, 대인 관계 능력, 협업 태도 등을<br/>평가하여 조직 적합성 판단
          </p>
        </button>
        ...
        <button 
          ...
          onClick={() => handleGenerateQuestions("지원 동기와 개인의 목표 및 가치관이 회사와 부합하는지 평가")}
        >
          <div className="flex justify-center mb-4">
          ...
          <p className="text-base text-gray-600 mt-2 text-center">
            지원 동기와 개인의 목표 및 가치관이<br/>회사와 부합하는지 평가
          </p>
        </button>
      </div>
    </div>
